...
*  SPI.
*/
    unsigned long ulDummy; // Variable to hold the "trash" data received from SPI

    // Set CS low
    MAP_SPICSEnable(GSPI_BASE);
    MAP_GPIOPinWrite(OLED_CS_BASE, OLED_CS_PIN, 0);

    // Set D/C pin LOW for Command Mode
    // 0 = Command, high = Data
    MAP_GPIOPinWrite(OLED_DC_BASE, OLED_DC_PIN, 0);

    // Send the byte via SPI
    // Arguments: Base, TxBuffer, RxBuffer, Length, Flags
    // Note: We pass &ulDummy because SPI always reads while writing.
    MAP_SPITransfer(GSPI_BASE, &c, (unsigned char*)&ulDummy, sizeof(c), 0);

    // Set CS high
    MAP_SPICSDisable(GSPI_BASE);
    MAP_GPIOPinWrite(OLED_CS_BASE, OLED_CS_PIN, OLED_CS_PIN);
...
*/
    ...
    // Set D/C pin HIGH for Data Mode
    MAP_GPIOPinWrite(OLED_DC_BASE, OLED_DC_PIN, OLED_DC_PIN);
